public class AES_CipherFeedback256 {
    ...
    private final int WB = 4; // Number of words in a block
    private final int AES_128_ROUNDS = 10;
    private final int AES_192_ROUNDS = 12;
    private final int AES_256_ROUNDS = 14;
    private final int AES_BLOCK_SIZE = 16;
    private final int[] AES_KEYS_IZES = {16, 24, 32};

    private int rounds;
    private byte[] prevKey = null;
    private int[] sessionKe = null; // key for encryption

    private byte[] k; // encrypt/decrypt output buffer
    private byte[] register;

    public void initialize(byte[] key, byte[] iv) {
        if ((key == null) || (iv == null) || (iv.length != AES_BLOCK_SIZE)) {
            System.out.println("Invalid key or initialization vector!");
            return;
        }
        if (key.length == AES_KEYS_IZES[0]) {
            rounds = AES_128_ROUNDS;
        } else if (key.length == AES_KEYS_IZES[1]) {
            rounds = AES_192_ROUNDS;
        } else if (key.length == AES_KEYS_IZES[2]) {
            rounds = AES_256_ROUNDS;
        } else {
            System.out.println("Invalid key length " + key.length);
            return;
        }
        k = new byte[AES_BLOCK_SIZE];
        register = new byte[AES_BLOCK_SIZE];
        for (int i = 0; i < AES_BLOCK_SIZE; i++) register[i] = iv[i];

        if (sessionKe != null) for (int i = 0; i < sessionKe.length; i++) sessionKe[i] = (byte) 0;
        sessionKe = genRoundKeys(key, rounds);
        prevKey = key.clone();
    }

    private int[] genRoundKeys(byte[] key, int rounds) {
    ...
    public void encryptBlock(byte[] p, int po, byte[] c, int co) {
        int[] session = sessionKe;
        int ti0, ti1, ti2, ti3;
        int a0, a1, a2, a3;
        int w = session.length - WB;
    ...
    private int encryptInternal(byte[] plain, int plainOffset, int plainLen, byte[] cipher, int cipherOffset) {
        int loopCount = plainLen / AES_BLOCK_SIZE;
        for (; loopCount > 0; plainOffset += AES_BLOCK_SIZE, cipherOffset += AES_BLOCK_SIZE, loopCount--) {
            encryptBlock(register, 0, k, 0);
            for (int i = 0; i < AES_BLOCK_SIZE; i++) {
                register[i] = cipher[i + cipherOffset] = (byte) (k[i] ^ plain[i + plainOffset]);
            }
        }
        return plainLen;
    }

    private int decryptInternal(byte[] cipher, int cipherOffset, int cipherLen, byte[] plain, int plainOffset) {
    ...
    public void encrypt(byte[] inputData, int inputDataLength, byte[] outputData) {
        int cipherOffset = 0;
        int plainOffset = 0;
        int oddBytes = inputDataLength % AES_BLOCK_SIZE;
        int len = encryptInternal(inputData, plainOffset, (inputDataLength - oddBytes), outputData, cipherOffset);
        plainOffset += len;
        cipherOffset += len;
        if (oddBytes != 0) {
            encryptBlock(register, 0, k, 0);
            for (int i = 0; i < oddBytes; i++) {
                outputData[i + cipherOffset] = (byte) (k[i] ^ inputData[i + plainOffset]);
            }
        }
    }

    public void decrypt(byte[] inputData, int inputDataLength, byte[] outputData) {
        int cipherOffset = 0;
        int plainOffset = 0;
        int oddBytes = inputDataLength % AES_BLOCK_SIZE;
        int len = decryptInternal(inputData, 0, (inputDataLength - oddBytes), outputData, plainOffset);
        cipherOffset += len;
        plainOffset += len;
        if (oddBytes != 0) {
            encryptBlock(register, 0, k, 0);
            for (int i = 0; i < oddBytes; i++) {
                outputData[i + plainOffset] = (byte) (inputData[i + cipherOffset] ^ k[i]);
            }
        }
    }
}